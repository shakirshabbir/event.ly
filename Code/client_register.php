...
<?php
	...
	if ( isset($_POST['clientRegisterFormSubmit']) )
	{
		...
		try{
			...
			$location = 'error.php?errorMsg='.urlencode( $e->getMessage()."<br />Unable to register this user!" );
    ...
    <body>
						...
						<div class="contact-form">			
							<form method="post" name="clientRegisterForm" action="">
								...
								<p>
									...
									<input pattern="{3,50}" name="clientLname" id="clientLname" type="text" placeholder="Last name" required />
								</p>
								<p>
		...
		<script type="text/javascript">
			...
			window.onload = function () {
				...
				//document.getElementById("clientLname").onchange = validateStringLength(3,50);
			}
			...
			function validateStringLength(min,max){
				alert('Thankyou');
				var string = document.getElementById("clientLname").value;
				if (string.length<3){
					document.getElementById("clientLname").setCustomValidity(string.length+":"+string+"Last Name should be minimum 3 characters!");
				}
				else if (string.length>50){
					document.getElementById("clientLname").setCustomValidity("Last Name can be maximum 50 characters!");
				}
				else
					document.getElementById("clientLname").setCustomValidity('');
