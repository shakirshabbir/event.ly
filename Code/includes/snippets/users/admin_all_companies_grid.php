...
*/
	$buttonText = "Add User";
	$fetchUsertypeId = 2; //Default: fetch all users
  ...
  <tr>
	...
	<?php
		...
		if ( $errorMessage!='' ) {//means some exception occured while fetching
			echo '<tr>';
			echo '<td data-th="Company" colspan="5">'.$errorMessage.'</td>';
			echo '</tr>';
			...
			foreach($results as $row) {
				echo '<tr>';
				echo '<td data-th="Company">'.$row['userFullName'].'</td>';
				...
				//UPDATE USER
				echo '<td data-th=""><a href="client_update.php?userId='.$row['userId'].'">Update</a>';
				//REMOVE USER
				echo '<td data-th="">';
